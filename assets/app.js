...
function renderArticle() {
  ...
  meta("description", post.summary); meta("og:title", post.title); meta("og:description", post.summary); if (post.cover) meta("og:image", new URL(post.cover, location.href).href); meta("article:published_time", `${post.date}T00:00:00+08:00`);
  ...
  structured.textContent = JSON.stringify({"@context":"https://schema.org","@type":"BlogPosting",headline:post.title,description:post.summary,datePublished:post.date,...(post.cover ? {image:new URL(post.cover, location.href).href} : {}),author:{"@type":"Person",name:authorName},publisher:{"@type":"Organization",name:siteName},inLanguage:"zh-CN"}); document.head.append(structured);
