...
const feed = `<?xml version="1.0" encoding="UTF-8"?>\n<rss version="2.0"><channel><title>知微</title><link>${siteUrl}</link><description>关于生活、阅读与创造的个人博客</description><language>zh-CN</language><lastBuildDate>${new Date().toUTCString()}</lastBuildDate>${feedPosts.map(post => { const url = post.url || `posts/${encodeURIComponent(post.slug)}.html`; return `<item><title>${xml(post.title)}</title><link>${siteUrl}/${xml(url)}</link><guid isPermaLink="true">${siteUrl}/${xml(url)}</guid><pubDate>${new Date(`${post.date}T00:00:00+08:00`).toUTCString()}</pubDate><description>${xml(post.summary)}</description></item>`; }).join("")}</channel></rss>`;
